...
class PCS_OOB(PCS_UQ):
    def __init__(
        ...
        self.disturb_method = disturb_method
        self.downsample_ratio = downsample_ratio
        if self.disturb_method == "downsample":
            if self.downsample_ratio is None:
                raise ValueError(
                    "downsample_ratio must be provided when disturb_method is downsample"
                )
            if self.downsample_ratio > 1 or self.downsample_ratio < 0:
                raise ValueError("downsample_ratio must be between 0 and 1")
            print(
                f"Using downsample method with downsample_ratio: {self.downsample_ratio}",
                flush=True,
            )
        elif self.disturb_method == "bootstrap":
            print(f"Using bootstrap method", flush=True)
        else:
            raise NotImplementedError(f"Invalid disturb_method: {self.disturb_method}")
        ...
        for model_name, model in self.top_k_models.items():
            ...
            for i in tqdm(
                ...
                if (
                    ...
                    n_samples = len(X)
                    if self.disturb_method == "bootstrap":
                        bootstrap_indices = rng.choice(
                            range(n_samples), size=n_samples, replace=True
                        )
                    elif self.disturb_method == "downsample":
                        bootstrap_indices = rng.choice(
                            range(n_samples),
                            size=int(n_samples * self.downsample_ratio),
                            replace=False,
                        )
                    else:
                        raise NotImplementedError(
                            f"Invalid disturb_method: {self.disturb_method}"
                        )
