...
class PCS_UQ:
    def __init__(
        ...
        assert calibration_method in ["multiplicative", "additive"], (
    ...
    def fit(self, X, y, alpha=None):
        ...
        uncalibrated_intervals = self.get_intervals(
            X_calib
        )  # get the uncalibrated intervals and raw width/coverage
        self.uncalibrated_metrics = get_all_metrics(
            y_calib, uncalibrated_intervals[:, [0, 2]]
        )  # drop median to assess raw coverage and width
        gamma = self.calibrate(
            uncalibrated_intervals, y_calib, self.calibration_method
    ...
    def get_intervals(self, X):
        ...
        intervals[:, 0] = np.quantile(
            all_predictions, self.alpha / 2, axis=1
        )  # Lower bound
        intervals[:, 1] = np.quantile(all_predictions, 0.5, axis=1)  # Median
        intervals[:, 2] = np.quantile(
            all_predictions, 1.0 - self.alpha / 2, axis=1
        )  # Upper bound
        return intervals
    ...
    def calibrate(
        ...
        if self.calibration_method == "multiplicative":
            return self._calibrate_multiplicative(
                intervals, y_calib, gamma_min, gamma_max, tol
            )
        elif self.calibration_method == "additive":
            return self._calibrate_additive(intervals, y_calib, step_size=tol)

    def _calibrate_multiplicative(
        self, intervals, y_calib, gamma_min=0, gamma_max=1000.0, tol=1e-6
    ):
        """
    ...
    def predict(
        ...
        if self.calibration_method == "multiplicative":
            lower_bound = uncalibrated_intervals[:, 1] - self.gamma * (
                uncalibrated_intervals[:, 1] - uncalibrated_intervals[:, 0]
            )
            upper_bound = uncalibrated_intervals[:, 1] + self.gamma * (
                uncalibrated_intervals[:, 2] - uncalibrated_intervals[:, 1]
            )
        elif self.calibration_method == "additive":
...
if __name__ == "__main__":
    ...
    for method in calibration_method:
        pcs_uq = PCS_UQ(
            models, save_path="test", load_models=True, calibration_method=method
        )
