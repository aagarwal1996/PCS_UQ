...
from src.conformal_methods.classification.multi_class_conformal import (
    MultiClassConformal,
)
from src.conformal_methods.classification.majority_vote_classifier import (
...
def get_conformal_methods(conformal_type, model_name="XGBoost", seed=0):
    if conformal_type == "split_conformal_raps":
        return MultiClassConformal(
            model=MODELS[model_name], seed=seed, conformity_score="RAPS"
        ), f"split_conformal_raps_{model_name}"
    elif conformal_type == "split_conformal_lac":
        return MultiClassConformal(
            model=MODELS[model_name], seed=seed, conformity_score="LAC"
        ), f"split_conformal_lac_{model_name}"
    elif conformal_type == "split_conformal_naive":
        return MultiClassConformal(
            model=MODELS[model_name], seed=seed, conformity_score="Naive"
        ), f"split_conformal_naive_{model_name}"
    elif conformal_type == "split_conformal_aps":
        return MultiClassConformal(
            model=MODELS[model_name], seed=seed, conformity_score="APS"
        ), f"split_conformal_aps_{model_name}"
    elif conformal_type == "split_conformal_topk":
        return MultiClassConformal(
            model=MODELS[model_name], seed=seed, conformity_score="TopK"
        ), f"split_conformal_topk_{model_name}"
    elif conformal_type == "majority_vote":
        return MajorityVoteClassifier(
            models=MODELS, seed=seed, conformity_score="APS"
        ), f"majority_vote"


def get_pcs_methods(pcs_type, seed=0):
    if pcs_type == "pcs_uq":
        return MultiClassPCS(
            models=MODELS,
            num_bootstraps=1000,
            alpha=0.1,
            top_k=1,
...
def get_classification_datasets(dataset_name):
    ...
    importance = pd.read_csv(
        f"experiments/data/classification/{dataset_name}/importances.csv"
    )
