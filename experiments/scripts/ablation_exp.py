...
def get_pcs_methods(
    pcs_type, seed=0, n_boot=1000, top_k=1, calibration_method="multiplicative"
):
    if pcs_type == "pcs_uq":
        return PCS_UQ(
            models=MODELS,
...
def get_subgroup_metrics(
    X_test_df, y_test, y_pred, bin_df_test, importance, method_name
):
    ...
    for imp_var in importance["feature"]:
        ...
        for subgroup in X_test_df_subgroup[f"subgroup_{imp_var}"].unique():
            ...
            subgroup_metrics[subgroup]["mean_width_scale"] = (
                subgroup_metrics[subgroup]["mean_width"] / range_y_test
            )
            subgroup_metrics[subgroup]["median_width_scale"] = (
                subgroup_metrics[subgroup]["median_width"] / range_y_test
            )
...
def run_ablation_regression_experiments(
    ...
    if ablation_exp == "split":
        (
            ...
            X, y, bin_df, X_df, train_size=train_size, random_state=seed
        )
        if X_train.shape[0] <= 5000:
            n_train = int(X_train.shape[0] * train_frac)
            X_train, y_train, bin_df_train, X_df_train = (
                X_train[:n_train],
                y_train[:n_train],
                bin_df_train[:n_train],
                X_df_train[:n_train],
            )
        else:
        ...
        X_train, y_train, bin_df_train, X_df_train = (
            X_train[:max_samples],
            y_train[:max_samples],
            bin_df_train[:max_samples],
            X_df_train[:max_samples],
        )
    else:
        raise ValueError(
            f"Invalid ablation experiment type '{ablation_exp}'. Must be 'split', 'boot', 'calib', 'model'."
        )
...
def agg_results(
    ...
    for dataset in datasets:
        ...
        all_files = list(
            dataset_path.glob(
                f"*_seed_*_train_size_{train_size}_nboot_{n_boot}_train_frac_{train_frac}_nmodel_{n_model}_calib_{calibration_method}_ablation_metrics.pkl"
            )
        )
        methods = set()
        ...
        for method in methods:
            ...
            method_files = list(
                dataset_path.glob(
                    f"{method}_seed_*_train_size_{train_size}_nboot_{n_boot}_train_frac_{train_frac}_nmodel_{n_model}_calib_{calibration_method}_ablation_metrics.pkl"
                )
            )
...
def agg_subgroup_results(
    ...
    for dataset in datasets:
        ...
        all_files = list(
            dataset_path.glob(
                f"*_seed_*_train_size_{train_size}_nboot_{n_boot}_train_frac_{train_frac}_nmodel_{n_model}_calib_{calibration_method}_ablation_subgroup_metrics.pkl"
            )
        )
        methods = set()
        for file in all_files:
        ...
        for method in methods:
            ...
            method_files = list(
                dataset_path.glob(
                    f"{method}_seed_*_train_size_{train_size}_nboot_{n_boot}_train_frac_{train_frac}_nmodel_{n_model}_calib_{calibration_method}_ablation_subgroup_metrics.pkl"
                )
            )
            ...
            for var in all_var_metrics[0].keys():
                ...
                for var_group in all_var_metrics[0][var].keys():
                    ...
                    for key in all_var_metrics[0][var][var_group].keys():
                        try:
                            values = [
                                m.get(var).get(var_group).get(key)
                                for m in all_var_metrics
                            ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n_boot", type=int, default=1000, help="Number of bootstraps for PCS methods"
    )
    parser.add_argument(
        "--abl_type",
        type=str,
        default="split",
        help="Type of ablation experiment ('split' or 'boot')",
    )
    parser.add_argument(
        "--train_frac",
        type=float,
        default=1,
        help="Fraction of training data to use for ablation experiments",
    )
    parser.add_argument(
        "--n_model", type=int, default=1, help="Number of models to use for PCS methods"
    )
    parser.add_argument(
        "--calibration_method",
        type=str,
        default="multiplicative",
        help="Calibration method for PCS methods",
    )
    ...
    run_ablation_regression_experiments(
        ...
        calibration_method=args.calibration_method,
        ablation_exp=args.abl_type,
        results_dir=f"experiments/results/ablation/{args.abl_type}",
    )
    agg_results(
        n_boot=args.n_boot,
        train_size=args.train_size,
