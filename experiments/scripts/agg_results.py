...
def agg_results_dataset_method(
    results_dir,
    dataset_name="data_parkinsons",
    ...
    for seed in SEEDS:
        try:
            seed_results_dir = Path(
                f"{results_dir}/{dataset_name}/{uq_method}{estimator}seed_{seed}_train_size_{train_size}_metrics.pkl"
            )
            results.append(pickle.load(open(seed_results_dir, "rb")))
        except:
            print(
                f"Error in loading results for seed {seed}: {results_dir}/{dataset_name}/{uq_method}{estimator}seed_{seed}_train_size_{train_size}_metrics.pkl"
            )
            pass
...
def agg_subgroup_results_dataset_method(
    results_dir,
    dataset_name="data_parkinsons",
    uq_method="split_conformal",
    estimator="XGBoost",
    ...
    for seed in SEEDS:
        try:
            seed_results_dir = Path(
                f"{results_dir}/{dataset_name}/{uq_method}{estimator}seed_{seed}_train_size_{train_size}_subgroup_metrics.pkl"
            )
            results.append(pickle.load(open(seed_results_dir, "rb")))
        except:
            print(
                f"Error in loading results for seed {seed} for dataset {dataset_name} and uq method {uq_method} and estimator {estimator} and train size {train_size}"
            )
            pass
    # Convert list of dictionaries to DataFrame
    ...
    for feature in features:
        ...
        for subgroup in subgroups:
            try:
                ...
                print(
                    f"Error in averaging results for feature {feature} and subgroup {subgroup}"
                )
                print(subgroup)
                pass
...
def get_all_uq_methods():
    ...
    for uq_method in VALID_UQ_METHODS:
        for estimator in VALID_ESTIMATORS:
            if uq_method not in SINGLE_MODEL_METHODS:
...
def main(task="regression", train_size=0.8):
    ...
    for dataset in DATASETS:
        ...
        for uq_method, estimator in uq_methods:
            results = agg_results_dataset_method(
        ...
        dataset_results_file = (
            aggregated_results_dir / f"{dataset}_train_size_{train_size}_results.pkl"
        )
    ...
    # Save all results
    all_results_file = (
        aggregated_results_dir / f"all_results_train_size_{train_size}.pkl"
    )
...
def agg_subgroup_results(task="regression", train_size=0.8):
    ...
    for dataset in DATASETS:
        results_dataset = {}
        for uq_method, estimator in uq_methods:
            results = agg_subgroup_results_dataset_method(
                results_dir=results_dir,
                dataset_name=dataset,
                uq_method=uq_method,
                estimator=estimator,
        ...
        # Save dataset-specific results
        dataset_results_file = (
            aggregated_results_dir
            / f"{dataset}_train_size_{train_size}_subgroup_results.pkl"
        )
        with open(dataset_results_file, "wb") as f:
            pickle.dump(results_dataset, f)
        all_results[dataset] = results_dataset
    all_results_file = (
        aggregated_results_dir / f"all_subgroup_results_train_size_{train_size}.pkl"
    )
    with open(all_results_file, "wb") as f:
        pickle.dump(all_results, f)
    return all_results
